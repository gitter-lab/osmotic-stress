    ...
    def runPCSF(self, msgpath, seed):
        ...
        RETURNS: edgeList - the contents of stdout from msgsteiner: a list of edges in the 
                 ...
                 info - the contents of stderr from msgsteiner: if all goes well, a report on
                             the optimization
        """
        weight = self.dirEdges['YOL054W']['YKL049C']
        print 'runPCSF start found edge YOL054W YKL049C: %s' % weight
        ...
        with open('msgsteiner_input.txt', 'w') as of:        
            input = tempfile.TemporaryFile()
            for item in lines:
                input.write(item)
                of.write(item)
            ...
            input.write('R DUMMY\n\n')
            of.write('W DUMMY 100.0\n')
            of.write('R DUMMY\n\n')

        weight = self.dirEdges['YOL054W']['YKL049C']
        print 'runPCSF middle found edge YOL054W YKL049C: %s' % weight
        ...
        with open('msgsteiner_args.txt', 'w') as of:
            of.write(' '.join(subprocArgs))
        ...
        weight = self.dirEdges['YOL054W']['YKL049C']
        print 'runPCSF end found edge YOL054W YKL049C: %s' % weight        
        
        with open('msgsteiner_edgeList.txt', 'w') as of:
...
class PCSFOutput(object):
    def __init__(self, inputObj, edgeList, info, outputpath, outputlabel, betweenness):
        ...
        weight = inputObj.dirEdges['YOL054W']['YKL049C']
        print 'PCSFOutput start found edge YOL054W YKL049C: %s' % weight
        ...
        for edge in edges:
            ...
            if len(words) > 0:
                ...
                try:
                    weight = inputObj.dirEdges['YOL054W']['YKL049C']
                    print 'PCSFOutput dirEdge check found edge YOL054W YKL049C: %s' % weight
